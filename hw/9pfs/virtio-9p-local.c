...
static int local_set_xattr(const char *path, FsCred *credp)
{
    ...
    if (credp->fc_uid != -1) {
        ...
        err = setxattr(path, "user.virtfs.uid", &tmp_uid, sizeof(uid_t), 0, 0, 0);
        if (err) {
            return err;
        }
    }
    if (credp->fc_gid != -1) {
        ...
        err = setxattr(path, "user.virtfs.gid", &tmp_gid, sizeof(gid_t), 0, 0, 0);
        if (err) {
            return err;
        }
    }
    if (credp->fc_mode != -1) {
        ...
        err = setxattr(path, "user.virtfs.mode", &tmp_mode, sizeof(mode_t), 0, 0, 0);
        if (err) {
            return err;
        }
    }
    if (credp->fc_rdev != -1) {
        ...
        err = setxattr(path, "user.virtfs.rdev", &tmp_rdev, sizeof(dev_t), 0, 0, 0);
        if (err) {
            return err;
        }
    }
