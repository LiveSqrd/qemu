...
static int local_set_xattr(const char *path, FsCred *credp)
{
    ...
    if (credp->fc_uid != -1) {
        ...
        err = setxattr(path, "user.virtfs.uid", &tmp_uid, sizeof(uid_t), 0, 0);
        if (err) {
            return err;
        }
    }
    if (credp->fc_gid != -1) {
        ...
        err = setxattr(path, "user.virtfs.gid", &tmp_gid, sizeof(gid_t), 0, 0);
        if (err) {
            return err;
        }
    }
    if (credp->fc_mode != -1) {
        ...
        err = setxattr(path, "user.virtfs.mode", &tmp_mode, sizeof(mode_t), 0, 0);
        if (err) {
            return err;
        }
    }
    if (credp->fc_rdev != -1) {
        ...
        err = setxattr(path, "user.virtfs.rdev", &tmp_rdev, sizeof(dev_t), 0, 0);
        if (err) {
            return err;
        }
    }
                       ...
                       V9fsFidOpenState *fs, struct stat *stbuf)
{
    ...
    if (fs_ctx->export_flags & V9FS_SM_MAPPED) {
        ...
        if (fgetxattr(fd, "user.virtfs.uid", &tmp_uid, sizeof(uid_t), 0, 0) > 0) {
            stbuf->st_uid = le32_to_cpu(tmp_uid);
        }
        if (fgetxattr(fd, "user.virtfs.gid", &tmp_gid, sizeof(gid_t), 0, 0) > 0) {
            stbuf->st_gid = le32_to_cpu(tmp_gid);
        }
        if (fgetxattr(fd, "user.virtfs.mode", &tmp_mode, sizeof(mode_t), 0, 0) > 0) {
            stbuf->st_mode = le32_to_cpu(tmp_mode);
        }
        if (fgetxattr(fd, "user.virtfs.rdev", &tmp_dev, sizeof(dev_t), 0, 0) > 0) {
            stbuf->st_rdev = le64_to_cpu(tmp_dev);
        }
    } else if (fs_ctx->export_flags & V9FS_SM_MAPPED_FILE) {
        errno = EOPNOTSUPP;
